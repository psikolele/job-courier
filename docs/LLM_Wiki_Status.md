...
**Prep-check (sessione 2, stesso giorno):** il repo **non usa Storage Vercel** (nessuna dep `@vercel/kv|postgres|blob|edge-config`), quindi l'import cross-account è solo codice + env vars. Le env vars sono 5; due non si copiano: `CRON_SECRET` va rigenerato e **`VERCEL_DEPLOY_HOOK_URL` va ricreato sul progetto nuovo** — è la trappola silenziosa, perché il cron notturno `/api/rebuild` non fallisce, chiama l'hook che trova, e col valore vecchio continuerebbe a ridispiegare il progetto che stiamo dismettendo. Nessun `*.vercel.app` nostro hardcoded nel codice. Da reimpostare da UI: root directory `webapp`, branch prod `main`, le 5 env. GitHub Actions non toccate. **Bloccante:** mail dedicata e nuovo account Vercel sono passi dell'utente, non ancora fatti.
